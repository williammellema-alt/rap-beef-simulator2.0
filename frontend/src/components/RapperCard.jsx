...
const RapperSearch = ({ side, rapper, open, setOpen, customRapper, setCustomRapper, rappers, onSelect, onCustomSubmit }) => (
      ...
      <PopoverContent className="w-full p-0 bg-[#121212] border-[#262626]" align="start">
        <Command className="bg-transparent">
          <CommandInput
            ...
            value={customRapper} onValueChange={setCustomRapper}
          />
          <CommandList>
            <CommandEmpty className="p-4">
              <div className="space-y-2">
                <p className="text-[#A3A3A3] text-sm">No rapper found.</p>
                <Button onClick={onCustomSubmit} className="w-full bg-[#FF3B30] hover:bg-[#D62B22] rounded-none" data-testid={`add-custom-rapper-${side}`}>
...
const EraSlider = ({ side, era, setEra, rapper, primeMarkerLeft }) => (
  <div className="space-y-4">
    <Label className="text-[#A3A3A3] font-body text-sm">PRIME ERA</Label>
    <div className="text-center">
      <div className="era-display text-[#FFD700]" data-testid={`era-display-${side}`}>{era}</div>
      <div className="text-[#A3A3A3] font-body text-sm uppercase tracking-wider">{getEraLabel(era)}</div>
    </div>
    <div className="slider-gold px-2 relative">
      {rapper && (
        ...
        >
          <div className="absolute -top-5 left-1/2 -translate-x-1/2 text-[#FF3B30] text-[10px] font-body whitespace-nowrap">PRIME</div>
        </div>
      )}
      <Slider
        value={[era]} min={1990} max={2025} step={1}
    ...
    <div className="flex justify-between text-xs text-[#525252] font-body">
      <span>1990</span><span>2025</span>
    </div>
  </div>
);

const SelectedRapperInfo = ({ rapper }) => (
  <motion.div {...FADE_IN} className="p-4 bg-[#1A1A1A] border border-[#262626]">
    <div className="font-heading text-3xl text-white uppercase tracking-tight">{rapper.name}</div>
    <div className="text-[#A3A3A3] font-body text-sm mt-1">Default Prime: {rapper.default_era}</div>
  </motion.div>
);

const RapperCard = ({ side, rapper, setRapper, era, setEra, rappers, customRapper, setCustomRapper }) => {
  ...
  return (
    <Card className="bg-[#121212] border-[#262626] card-gritty relative z-10">
      ...
      <CardContent className="space-y-6">
        <RapperSearch
          side={side} rapper={rapper} open={open} setOpen={setOpen}
          customRapper={customRapper} setCustomRapper={setCustomRapper}
          rappers={rappers} onSelect={handleSelect} onCustomSubmit={handleCustomSubmit}
        />
        <EraSlider side={side} era={era} setEra={setEra} rapper={rapper} primeMarkerLeft={primeMarkerLeft} />
        {rapper && <SelectedRapperInfo rapper={rapper} />}
      </CardContent>
    </Card>
  );
};
